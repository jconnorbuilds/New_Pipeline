...
import { NewClientForm } from './pipeline_functions.js';
import $ from 'jquery';
...
import { drawNewRow, removeRow } from './pipeline-ui-funcs.js';
...
const handleStatusUpdateSuccess = (table) => (response) => {
  response.status === 'success'
    ? handleNewRowDraw(table, response.data)
    : console.error(response.message);
};

const handleStatusUpdateError = (table) => (response) => {
...
const handleStatusUpdate = (status, rowID) => {
  ...
  $.ajax({
    ...
    data: { status: status },
    dataType: 'json',
    success: () => handleStatusUpdateSuccess(PipelineDT.getTable()),
    error: () => handleStatusUpdateError(PipelineDT.getTable()),
  });
};
...
export {
  ...
  handleStatusUpdateSuccess as handleSuccessResponse,
  handleStatusUpdateError as handleErrorResponse,
