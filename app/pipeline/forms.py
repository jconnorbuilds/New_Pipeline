...
for y in range(date.today().year, 2021, -1):
    for m in range(12, 0, -1):
        DATE_CHOICES.append(
            (f'{y}-{m:02d}-01', f'{calendar.month_abbr[m]} {y}'))
...
class AddVendorToCostForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        sorted_vendors = sorted(Vendor.objects.all(),
                                key=lambda vendor: vendor.familiar_name)
        VENDOR_CHOICES = [('', 'Select vendor...')] + [(vendor.pk,
                                                        vendor.familiar_name) for vendor in sorted_vendors]
...
class UpdateCostForm(forms.Form):
    ...
    ), required=False, empty_label='Select vendor', widget=forms.Select(attrs={'class': 'form-select'}))
    cost = forms.ModelChoiceField(queryset=Cost.objects.all())
    invoice_status = forms.ChoiceField(required=False, widget=forms.Select(
        attrs={'class': 'form-select'}), choices=Cost.INVOICE_STATUS_CHOICES)
...
class JobForm(ModelForm):
    ...
    granular_revenue = forms.BooleanField(
        widget=forms.HiddenInput(), required=False)

    class Meta:
        model = Job
        fields = ['job_name', 'client', 'job_type', 'granular_revenue',
                  'revenue', 'add_consumption_tax', 'personInCharge',]
...
class ClientForm(ModelForm):
    class Meta:
        model = Client
        fields = ['friendly_name', 'proper_name',
                  'proper_name_japanese', 'job_code_prefix']
...
class PipelineBulkActionsForm(forms.Form):
    actions = forms.CharField(max_length=50,
                              widget=forms.Select(choices=(('NEXT', 'Move to next month'),
                                                           ('PREVIOUS',
