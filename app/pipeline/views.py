...
class PipelineViewBase(LoginRequiredMixin, SuccessMessageMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['headers'] = ["", "クライアント名", "案件名", "ジョブコード", "収入（税込）", "出費",
                              "利益率（税抜）", "請求期間", "種目", "ステータス", "入金日", "Invoice Info Completed", ]
        return context
    ...
    def post(self, request, *args, **kwargs):
        if 'bulk-actions' in request.POST:
            ...
            checked_jobs = Job.objects.filter(
                id__in=request.POST.getlist('select'))
            if bulk_form.is_valid():
                ...
                if action == "NEXT":
                    ...
                    if i > 0:
                        messages.warning(
                            request, f'{i} jobs were deleted. They can be restored from the admin panel.')
                    else:
            ...
            if request.method == "POST":  # TODO : I don't think this if statement is necessary? double check
                ...
                if form.is_valid():
                    ...
                    if response["valid_template"] == True:
                        ...
                        if created_items:
                            messages.success(
                                request, f'{len(created_items)} jobs were added successfully!')

                        if not_created_items:
                            messages.info(
                                request, f'{len(not_created_items)} items were already in the database, so they were left alone.')
            ...
            if invoice_file and invoice_file.size < 100 * 1024 * 1024:
                if invoice_file.size < 100 * 1024 * 1024:
                    try:
                        ...
                        full_filepath = (invoice_folder + "/" + date_folder_name + "/" +
                                         currency_folder_name + "/" + cost.PO_number + file_extension)
...
def upload_invoice_confirmation_email(request):
    ...
    """
    successful_invoice_ids = json.loads(
        request.session.get('successful_invoices'))
    unsuccessful_invoice_ids = json.loads(
        request.session.get('unsuccessful_invoices'))

    successful_invoices = Cost.objects.filter(id__in=successful_invoice_ids)
    unsuccessful_invoices = Cost.objects.filter(
        id__in=unsuccessful_invoice_ids)
    context = {'successful_invoices': successful_invoices,
    ...
    recipient_list = [vendor.email] if not settings.DEBUG else [
        "joe@bwcatmusic.com"]
...
def invoice_upload_view(request, vendor_uuid):
    ...
    requested_invoices = Cost.objects.filter(
        vendor_id=vendor.id, invoice_status='REQ').select_related('job')
    jobs = list(Job.objects.filter(cost_rel__in=requested_invoices).values(
    ...
    context = {'requested_invoices': requested_invoices, 'invoices_json': json.dumps(
        invoices_json), 'jobs_json': jobs_json, 'vendor_id': vendor.id}
...
def jobs_csv_export(request):
    ...
    if csv_export_form.is_valid():
        ...
        scope = Job.objects.filter(
            job_date__gte=fromDate, job_date__lt=thruDate)
        print(scope)
        ...
        for job in scope:
            ...
            data = [client_name, job.job_name, job.job_code,
                    f'{job.revenue:,}', f'{job.total_cost:,}', job.job_type, date_MY]
            ...
            with open(csvfile, newline='') as templateCSV:
                ...
                for row in reader:
                    ...
                    invoices_from_vendor = [
                        invoice for invoice in invoices if invoice.vendor.payment_id == recipient_id]
                    for invoice in invoices_from_vendor:
                        ...
                        if invoice.currency != row[target_currency_idx]:
                            processing_status[invoice.PO_number] = {
                                "status": "error", "message": f"Currency mismatch. Invoices for this vendor should be in {row[target_currency_idx]}."}
                            continue

                        upper_limit_for_JPY = 950000
                        approx_amount_in_JPY = invoice.amount * \
                            forex_rates[row[target_currency_idx]]
                        ...
                        if approx_amount_in_JPY > upper_limit_for_JPY:
                            ...
                            invoice.save()
                            processing_status[invoice.PO_number] = {
                                "status": "success", "message": f"Successfully processed as {split_into} payments!"}
                            ...
                            invoice.save()
                            processing_status[invoice.PO_number] = {
                                "status": "success", "message": "Successfully processed!"}
                ...
                try:
                    ...
                    if created:
                        created_items.append(
                            f'{friendly_name} - {job_code_prefix}')
                    elif not created:
                    ...
                    messages.error(
                        request, f"{friendly_name} - {job_code_prefix} wasn't added.\n{e}")
                    ...
                    print(e)
                    messages.error(
                        request, f"{friendly_name} - {job_code_prefix} wasn't added - something bad happened! \n{e}")
    ...
    with open('static/pipeline/vendors.csv', 'r') as myFile:
        ...
        for row in reader:
            ...
            try:
                ...
                messages.error(
                    request, f'{familiar_name} {email if email else ""} was not added.\n{e}')
            except NameError as n:
                ...
                print(e)
                messages.error(
                    request, f'{familiar_name} {email if email else ""} was not added - something bad happened! \n{e}')
        ...
        if not_created_items:
            messages.info(
                request, f'{len(not_created_items)} vendors were already in the database, so they were left alone. If you need to update vendor information, go to Vendors -> View Vendors and click the update link.')
...
class CostCreateView(LoginRequiredMixin, CreateView):
    model = Cost
    fields = ['vendor', 'description', 'amount',
              'currency', 'invoice_status', 'notes']
    ...
    def get_context_data(self, **kwargs):
        ...
        context['headers'] = [
            "金額(¥)", "金額(現地)", "ベンダー名", "作業の内容", "PO番号", "請求書ステータス", "", "編集", "ID"]
...
class CostUpdateView(RedirectToPreviousMixin, UpdateView):
    ...
    fields = ['vendor', 'description', 'amount', 'currency', 'invoice_status', 'notes',
              'locked_exchange_rate', 'exchange_rate_locked_at', 'exchange_rate_override']
...
class ClientCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    model = Client
    fields = ["friendly_name", "job_code_prefix", "proper_name",
              "proper_name_japanese", "paymentTerm", "notes"]
...
class VendorListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        sorted_vendors = sorted(
            queryset, key=lambda vendor: vendor.familiar_name)
...
class VendorUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    fields = ["first_name", "last_name", "email", "vendor_code",
              "use_company_name", "company_name", "notes", "payment_id"]
...
class ClientUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    fields = ["proper_name", "proper_name_japanese",
              "friendly_name", "job_code_prefix",]
